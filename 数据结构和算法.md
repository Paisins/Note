...
class BST:
    ...
    def find_value(node, type_):
        if type_ == 'min':
            if node.left:
                return find_value(node.left, 'min')
            else:
                return node, node.value
        else:
            if node.right:
                return find_value(node.right, 'max')
            else:
                return node, node.value
    ...
    def delete(self, node):
        # 找到被删除的叶子结点和值
        if node.left:
            leaf_node, value = find_value(node.left, 'max')
        elif node.right:
            leaf_node, value = find_value(node.left, 'min')
        else:
